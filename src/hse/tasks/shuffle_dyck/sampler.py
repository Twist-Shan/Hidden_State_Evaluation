from __future__ import annotations

from dataclasses import dataclass

import torch

from .config import ShuffleDyckConfig


@dataclass
class ShuffleDyckBatch:
    tokens: torch.Tensor
    dyck_mask: torch.Tensor
    dyck_steps: torch.Tensor
    noise_tokens: torch.Tensor
    bracket_type_ids: torch.Tensor


class ShuffleDyckSampler:
    """Generate interleavings of several balanced bracket streams."""

    def __init__(self, config: ShuffleDyckConfig, seed: int | None = None):
        self.config = config
        self.device = torch.device(config.device)
        self.generator = torch.Generator(device=self.device)
        if seed is not None:
            self.generator.manual_seed(int(seed))

    @property
    def vocab_size(self) -> int:
        return self.config.vocab_size

    def sample(self, batch_size: int) -> ShuffleDyckBatch:
        cfg = self.config
        tokens = torch.randint(
            low=0,
            high=cfg.num_noise_tokens,
            size=(batch_size, cfg.seq_len),
            generator=self.generator,
            device=self.device,
        )
        noise_tokens = tokens.clone()
        type_order = self._sample_type_order(batch_size)
        stream_steps = self._sample_stream_steps(batch_size)
        dyck_steps, bracket_type_ids = self._materialize_interleaving(type_order, stream_steps)
        positions = self._sample_plant_positions(batch_size)
        dyck_mask = torch.zeros(batch_size, cfg.seq_len, dtype=torch.bool, device=self.device)

        rows = torch.arange(batch_size, device=self.device).unsqueeze(1)
        dyck_mask[rows, positions] = True

        bracket_tokens = torch.empty(batch_size, cfg.total_length, dtype=torch.long, device=self.device)
        for type_id, (open_tok, close_tok) in enumerate(zip(cfg.open_tokens, cfg.close_tokens)):
            is_type = bracket_type_ids == type_id
            bracket_tokens[is_type] = torch.where(
                dyck_steps[is_type] > 0,
                torch.full_like(dyck_steps[is_type], open_tok),
                torch.full_like(dyck_steps[is_type], close_tok),
            )
        tokens[rows, positions] = bracket_tokens

        return ShuffleDyckBatch(
            tokens=tokens,
            dyck_mask=dyck_mask,
            dyck_steps=dyck_steps,
            noise_tokens=noise_tokens,
            bracket_type_ids=bracket_type_ids,
        )

    def _sample_plant_positions(self, batch_size: int) -> torch.Tensor:
        cfg = self.config
        if cfg.generation_prob >= 1.0:
            base = torch.arange(cfg.total_length, device=self.device)
            return base.unsqueeze(0).expand(batch_size, -1)

        positions = []
        all_pos = torch.arange(cfg.seq_len, device=self.device)
        for _ in range(batch_size):
            mask = torch.rand(cfg.seq_len, generator=self.generator, device=self.device) < cfg.generation_prob
            chosen = all_pos[mask]
            if chosen.numel() < cfg.total_length:
                remaining = all_pos[~mask]
                perm = remaining[torch.randperm(remaining.numel(), generator=self.generator, device=self.device)]
                chosen = torch.cat([chosen, perm[: cfg.total_length - chosen.numel()]])
            elif chosen.numel() > cfg.total_length:
                perm = torch.randperm(chosen.numel(), generator=self.generator, device=self.device)
                chosen = chosen[perm[: cfg.total_length]]
            positions.append(chosen.sort().values)
        return torch.stack(positions, dim=0)

    def _sample_type_order(self, batch_size: int) -> torch.Tensor:
        cfg = self.config
        remaining = torch.full(
            (batch_size, cfg.num_bracket_types),
            cfg.steps_per_type,
            dtype=torch.long,
            device=self.device,
        )
        order = torch.empty(batch_size, cfg.total_length, dtype=torch.long, device=self.device)
        rows = torch.arange(batch_size, device=self.device)
        for t in range(cfg.total_length):
            active = remaining > 0
            probs = active.float()
            probs = probs / probs.sum(dim=1, keepdim=True)
            choice = torch.multinomial(probs, num_samples=1, replacement=True, generator=self.generator).squeeze(1)
            order[:, t] = choice
            remaining[rows, choice] -= 1
        return order

    def _sample_stream_steps(self, batch_size: int) -> torch.Tensor:
        cfg = self.config
        streams = []
        for _ in range(cfg.num_bracket_types):
            streams.append(self._sample_balanced_steps(batch_size, cfg.steps_per_type))
        return torch.stack(streams, dim=1)

    def _materialize_interleaving(
        self,
        type_order: torch.Tensor,
        stream_steps: torch.Tensor,
    ) -> tuple[torch.Tensor, torch.Tensor]:
        cfg = self.config
        rows = torch.arange(type_order.size(0), device=self.device)
        cursors = torch.zeros(type_order.size(0), cfg.num_bracket_types, dtype=torch.long, device=self.device)
        dyck_steps = torch.empty(type_order.size(0), cfg.total_length, dtype=torch.long, device=self.device)
        bracket_type_ids = torch.empty_like(dyck_steps)

        for t in range(cfg.total_length):
            type_ids = type_order[:, t]
            step_index = cursors[rows, type_ids]
            dyck_steps[:, t] = stream_steps[rows, type_ids, step_index]
            bracket_type_ids[:, t] = type_ids
            cursors[rows, type_ids] += 1
        return dyck_steps, bracket_type_ids

    def _sample_balanced_steps(self, batch_size: int, length: int) -> torch.Tensor:
        path = torch.empty(batch_size, length, dtype=torch.long, device=self.device)
        height = torch.zeros(batch_size, dtype=torch.long, device=self.device)
        opens_used = torch.zeros(batch_size, dtype=torch.long, device=self.device)
        max_opens = length // 2

        for t in range(length):
            remaining = length - t
            must_close = opens_used >= max_opens
            must_open = height <= 0
            can_random = ~(must_close | must_open)
            random_open = torch.rand(batch_size, generator=self.generator, device=self.device) < 0.5
            step_is_open = torch.where(can_random, random_open, must_open)
            step_is_open = torch.where(remaining <= height, torch.zeros_like(step_is_open), step_is_open)

            step = torch.where(step_is_open, torch.ones_like(height), -torch.ones_like(height))
            path[:, t] = step
            height += step
            opens_used += step_is_open.long()
        return path
